...
int api68_process(api68_t * api, void * buf16st, int n, const int interpolation, const int stereo_join)
{
  int ret = API68_IDLE;
  u32 * buffer;
  int buflen;
...
/*     } */
	...
	buflen = api->mix.stdbuflen;
        PL_mix(buffer, api->reg.mem, buflen, interpolation);
        if (stereo_join)
        {
        ...
        {
        	SC68mixer_blend_LR(buffer, buffer, buflen,0, 0, 0);
        }
      } else {
